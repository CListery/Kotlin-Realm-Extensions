...
inline fun <reified T : RealmModel> querySortedAsSingle(fieldName: List<String>, order: List<Sort>, noinline query: Query<T>? = null) =
    singleQuery(fieldName, order, query)
...
inline fun <reified T : RealmModel> querySortedAsSingle(fieldName: String, order: Sort, noinline query: Query<T>? = null) =
    singleQuery(listOf(fieldName), listOf(order), query)

/**
 * INTERNAL FUNCTIONS
 */
@PublishedApi
internal inline fun <reified T : RealmModel> singleQuery(fieldName: List<String>? = null, order: List<Sort>? = null, noinline query: Query<T>? = null) =
    performSingleQuery(fieldName, order, query, T::class.java)

private fun <T : RealmModel> T.singleQuery(fieldName: List<String>? = null, order: List<Sort>? = null, query: Query<T>? = null) =
...
internal fun <T : RealmModel> performSingleQuery(
    ...
    return Single.create<List<T>> { emitter ->
        ...
        val realm = RealmConfigManager.findConfig(javaClass).realm()
        ...
        result.addChangeListener { it ->
            emitter.onSuccess(realm.copy(it))
        }
